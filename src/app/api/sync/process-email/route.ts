...
export async function POST(req: Request) {
  ...
  try {
    ...
    const savedCount = await syncEmailTransactions(db, userId, [email], userWallets);

    if (savedCount === 0) {
      return NextResponse.json({ error: 'No transactions were found or saved from this email.' }, { status: 400 });
    }
